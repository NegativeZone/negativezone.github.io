...
Same thing here. An agent judging its own completion from inside the conversation subdivides forever: one more edge case, one more offer. Give it a finish line it doesn't control and the regress collapses. Tests green. Exit code zero. The first time you watch a harness stop an agent cold, it's quite something. Bonus when it also doesn't follow up with "Hey, do you also want to update the documentation? Just say the word."
